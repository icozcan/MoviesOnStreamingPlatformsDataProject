...
class StreamingDataAnalysis:
    ...
    def analyze_quality_scores(self):
        ...
        try:
            ...
            z_score = (statistic - (n_netflix * n_disney / 2)) / np.sqrt(n_netflix * n_disney * (n_netflix + n_disney + 1) / 12)
    ...
    def generate_visualizations(self):
        ...
        now including a separate RT Score vs. Age Category analysis.
        """
        try:
            ...
            bars_netflix = ax1.bar(x_positions - bar_width / 2,
                                   netflix_age_counts,
                                   bar_width,
                                   label='Netflix',
                                   color='#E50914')
            bars_disney = ax1.bar(x_positions + bar_width / 2,
                                  disney_age_counts,
                                  bar_width,
                                  label='Disney+',
                                  color='#113CCF')
            ...
            # 1.4: (Optional) Another chart or a placeholder
            ax4.set_title('1.4 Additional Chart / Placeholder')
            ax4.axis('off')

            plt.tight_layout()

            # === Figure 2: Quality Score Analysis ===
            # We'll use a 2 x 3 layout for deeper segmentation
            ...
            violin_parts = ax_q1.violinplot([netflix_rt_scores, disney_rt_scores],
            ...
            ax_q1.text(
                ...
                fontsize=7,
                bbox=dict(facecolor='white', edgecolor='black', boxstyle='round', alpha=0.8, pad=0.5),
                verticalalignment='top',
            ...
            df_platform_scores = (
                self.streaming_df
                # Filtre: RT_Score mevcut ve Netflix=1 veya Disney+=1
            ...
            # 2.6: **RT Scores by Age Category** (NEW)
            # Bu sayede, yaş kategorilerinin RT puanlarına etkisini ayrı bir grafikte görüyoruz.
            # Her bir Age_Ordinal için RT_Score değerlerini bir kutu grafikte gösteriyoruz.
            age_scores_data = []
            labels_for_age = []
            for cat in self.age_categories:
                cat_scores = self.streaming_df[self.streaming_df['Age_Ordinal'] == cat]['RT_Score'].dropna()
                # Boş olmayan kategorileri ekleyelim
                if len(cat_scores) > 0:
                    age_scores_data.append(cat_scores)
                    labels_for_age.append(cat)

            # Eğer hiç veri yoksa, kısaca bilgilendirme yapabiliriz
            if age_scores_data:
